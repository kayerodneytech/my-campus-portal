...
$requiredFields = [
    'first_name', 'last_name', 'email', 'password', 'course_id', 'payment_method',
    'date_of_birth', 'gender', 'address', 'nationality', 'id_number',
    'emergency_contact', 'emergency_phone', 'level'
];
...
try {
    // Insert into users table (NO phone field)
    $userStmt = $conn->prepare("
        INSERT INTO users (first_name, last_name, email, password, role, status)
        VALUES (?, ?, ?, ?, 'student', 'active')
    ");
    $userStmt->bind_param("ssss", $input['first_name'], $input['last_name'], $input['email'], $hashedPassword);
    ...
    // Insert into students table (phone is here)
    $studentStmt = $conn->prepare("
        INSERT INTO students (
            user_id, application_number, level, date_of_birth, gender, address,
            nationality, id_number, emergency_contact, emergency_phone, phone, status
        )
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
    ");
    $studentStmt->bind_param(
        ...
        $input['phone'] // Phone is stored in students table
    );
    ...
    $courseAppStmt = $conn->prepare("
        INSERT INTO course_applications (student_id, course_id, status, notes)
        VALUES (?, ?, 'pending', ?)
    ");
    ...
    $paymentStmt = $conn->prepare("
        ...
        VALUES (?, 50.00, ?, 'registration_fee', ?, 'completed', NOW())
    ");
