...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    fputcsv($output, ['Application Number', 'First Name', 'Last Name', 'Email', 'Phone', 'Level', 'Status', 'Created At']);
    
    $export_query = "SELECT s.application_number, u.first_name, u.last_name, u.email, u.phone, s.level, s.status, s.created_at 
                     FROM users u 
...
if (!empty($search_query)) {
    $query .= " AND (u.first_name LIKE ? OR u.last_name LIKE ? OR u.email LIKE ? OR s.application_number LIKE ?)";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_status'])) {
    ...
    if (in_array($new_status, $valid_statuses)) {
        $update_stmt = $conn->prepare("UPDATE students SET status = ?, updated_at = NOW() WHERE user_id = ?");
        $update_stmt->bind_param("si", $new_status, $user_id);
        
        if ($update_stmt->execute()) {
            // If activating student, also activate user account
            if ($new_status === 'active') {
                $activate_user = $conn->prepare("UPDATE users SET status = 'active' WHERE id = ?");
            ...
            if (!empty($notes)) {
                $note_stmt = $conn->prepare("INSERT INTO student_notes (student_id, admin_id, note) VALUES (?, ?, ?)");
            ...
            $error = "Error updating student status: " . $update_stmt->error;
        }
        $update_stmt->close();
        
        // Refresh page to show updated data
        header("Location: students.php?status=$status_filter&search=" . urlencode($search_query));
        exit();
    }
}
?>
...
<body class="bg-gray-100 min-h-screen">
                            ...
                            <?php foreach ($students as $student): ?>
                                <tr class="hover:bg-gray-50">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="text-sm font-medium text-gray-900"><?php echo htmlspecialchars($student['application_number']); ?></span>
                                    </td>
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <div class="flex items-center">
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        <span class="px-2 py-1 text-xs font-medium rounded-full 
                                            ...
                                            <?php echo $student['status'] == 'pending' ? 'bg-yellow-100 text-yellow-800' : ''; ?>
                                            <?php echo $student['status'] == 'suspended' ? 'bg-red-100 text-red-800' : ''; ?>
                                            <?php echo $student['status'] == 'rejected' ? 'bg-gray-100 text-gray-800' : ''; ?>">
    ...
    <!-- Student Detail Modal (will be populated by JavaScript) -->
